...
# Añadir al inicio de tu app.py después de las importaciones existentes
...
class DualLogger:
    ...
    def log(self, level, message):
        ...
        # Escribir a consola (para Render logs)
        print(log_entry)
        
        # Escribir a archivo (para email monitoring)
        try:
...
logging.basicConfig(
    ...
    stream=sys.stdout  # Asegurar que vaya a stdout para Render
)
...
def periodic_log_check():
    ...
    try:
        ...
        dual_logger.error(f"Error en check periódico de logs: {str(e)}")
    
    # Programar la próxima ejecución (10 minutos)
    timer = Timer(600.0, periodic_log_check)  # 600 segundos = 10 minutos
    timer.daemon = True
    timer.start()
...
def index():
    ...
    try:
        dual_logger.info("=== ACCESO A PÁGINA PRINCIPAL ===")
        dual_logger.info(f"IP del cliente: {request.remote_addr}")
        dual_logger.info(f"User-Agent: {request.headers.get('User-Agent', 'No disponible')}")
        
        if not app_ready:
            dual_logger.warning("⚠️ INTENTO DE ACCESO CON MODELOS NO CARGADOS")
            dual_logger.error("Los modelos de predicción no están disponibles")
        ...
        dual_logger.info(f"✅ Página principal cargada exitosamente")
        dual_logger.info(f"Modelos disponibles: {context['models_count']}")
        dual_logger.info(f"Características disponibles: {context['features_count']}")
        
        return render_template('index.html', **context)
        
    except Exception as e:
        dual_logger.error(f"❌ ERROR EN PÁGINA PRINCIPAL: {str(e)}")
        dual_logger.error(f"Traceback completo: {str(e)}")
...
def prediccion_page():
    ...
    try:
        dual_logger.info("=== ACCESO A PÁGINA DE PREDICCIÓN ===")
        dual_logger.info(f"IP del cliente: {request.remote_addr}")
        dual_logger.info(f"Referrer: {request.headers.get('Referer', 'Acceso directo')}")
        
        if not app_ready:
            dual_logger.warning("⚠️ INTENTO DE ACCESO A PREDICCIÓN CON MODELOS NO CARGADOS")
            dual_logger.error("Los modelos de predicción no están disponibles para formulario")
            return render_template('error.html', 
                                   error="Los modelos de predicción no están disponibles. Por favor, contacte al administrador."), 503
        ...
        dual_logger.info(f"Obteniendo características del modelo...")
        dual_logger.info(f"Características encontradas: {len(feature_names)}")
        dual_logger.info(f"Modelos disponibles: {available_models}")
        
        if not feature_names:
            dual_logger.error("❌ ERROR: No se pudieron obtener los nombres de las características")
        ...
        dual_logger.info(f"✅ Página de predicción cargada exitosamente")
        dual_logger.info(f"Modelo por defecto: {context['default_model']}")
        dual_logger.info(f"Información de características preparada para {len(feature_info)} campos")
        
        return render_template('prediccion.html', **context)
        
    except Exception as e:
        dual_logger.error(f"❌ ERROR EN PÁGINA DE PREDICCIÓN: {str(e)}")
        dual_logger.error(f"Detalles del error: {str(e)}")
...
# =============================================================================
# RUTAS DE LA APLICACIÓN CON LOGGING MEJORADO
# =============================================================================
...
# =============================================================================
# API ENDPOINTS CON LOGGING MEJORADO
# =============================================================================

@app.route('/api/predict', methods=['POST'])
def api_predict():
    """API endpoint para hacer predicciones (versión con monitoreo y logging)"""
    dual_logger.info("=== NUEVA SOLICITUD DE PREDICCIÓN API ===")
    dual_logger.info(f"IP del cliente: {request.remote_addr}")
    dual_logger.info(f"Método: {request.method}")
    dual_logger.info(f"Content-Type: {request.headers.get('Content-Type', 'No especificado')}")
    
    if not app_ready:
        dual_logger.error("❌ PREDICCIÓN RECHAZADA: Modelos no disponibles")
        return jsonify({
            'success': False,
    ...
    try:
        data = request.json
        dual_logger.info(f"Datos recibidos: {data is not None}")
        
        if not data:
            dual_logger.warning("⚠️ PREDICCIÓN RECHAZADA: No se recibieron datos")
            return jsonify({
                'success': False,
        ...
        dual_logger.info(f"Características recibidas: {len(features) if features else 0}")
        dual_logger.info(f"Modelo solicitado: {model_type}")
        dual_logger.info(f"Valores de características: {features}")
        
        if not features or not model_type:
            dual_logger.warning("⚠️ PREDICCIÓN RECHAZADA: Faltan parámetros requeridos")
            return jsonify({
                'success': False,
        ...
        if model_type not in available_models:
            dual_logger.error(f"❌ MODELO NO DISPONIBLE: {model_type}")
            return jsonify({
                'success': False,
        ...
        dual_logger.info(f"Características esperadas: {len(expected_features)}")
        
        if len(features) != len(expected_features):
            dual_logger.error(f"❌ ERROR DE VALIDACIÓN: Se esperan {len(expected_features)} características, se recibieron {len(features)}")
            return jsonify({
                'success': False,
        ...
        dual_logger.info("Procesando características para predicción...")
        feature_df = pd.DataFrame([features], columns=expected_features)
        dual_logger.info(f"DataFrame creado: {feature_df.shape}")
        
        # Hacer predicción
        dual_logger.info(f"Ejecutando predicción con modelo: {model_type}")
        prediction = predictor.predict(feature_df, model_type)
        dual_logger.info(f"Predicción realizada: {prediction}")
        
        # Formatear precio (convertir de miles a dólares)
        formatted_price = f"${prediction * 1000:,.2f}"
        dual_logger.info(f"Precio formateado: {formatted_price}")
        ...
        dual_logger.info(f"Intervalo de confianza: {confidence_interval}")
        
        # === REGISTRO PARA MONITOREO ===
        try:
            dual_logger.info("Registrando predicción para monitoreo...")
            ...
            dual_logger.info(f"Predicción registrada con ID: {prediction_id}")
            
        except Exception as monitor_error:
            dual_logger.warning(f"⚠️ Error registrando predicción para monitoreo: {str(monitor_error)}")
        ...
        dual_logger.info("✅ PREDICCIÓN COMPLETADA EXITOSAMENTE")
        dual_logger.info(f"Resultado final: {response_data}")
        
        return jsonify(response_data)
        
    except Exception as e:
        dual_logger.error(f"❌ ERROR CRÍTICO EN PREDICCIÓN: {str(e)}")
        dual_logger.error(f"Tipo de error: {type(e).__name__}")
        logger.error(f"Error en predicción: {str(e)}")
        return jsonify({
            'success': False,
...
def api_health():
    """Endpoint de verificación de salud con logging mejorado"""
    ...
    try:
        ...
        dual_logger.info(f"Verificando estado de la aplicación...")
        dual_logger.info(f"- App ready: {app_ready}")
        dual_logger.info(f"- Modelos cargados: {models_count}")
        dual_logger.info(f"- Predictor ready: {predictor_ready}")
        dual_logger.info(f"- Modelos disponibles: {available_models}")
        
        # Determinar el estado general
        overall_status = 'healthy' if (app_ready and predictor_ready and models_count > 0) else 'unhealthy'
        
        # Verificaciones adicionales para el logging
        checks = {
            'app_ready': app_ready,
            'predictor_ready': predictor_ready,
            'models_loaded': models_count > 0,
            'dual_logger_active': True,
            'monitoring_active': monitor is not None
        }
        
        dual_logger.info(f"Verificaciones completadas: {checks}")
        ...
        response_data = {
            ...
            'predictor_ready': predictor_ready,
            'checks': checks,
            'logging_system': 'dual_logger_active',
            'uptime_check': True
        }
        
        # Log del resultado
        if overall_status == 'healthy':
            dual_logger.info("✅ HEALTH CHECK EXITOSO - Sistema funcionando correctamente")
        else:
            dual_logger.warning("⚠️ HEALTH CHECK CON PROBLEMAS - Sistema no completamente saludable")
            dual_logger.warning(f"Problemas detectados: app_ready={app_ready}, predictor_ready={predictor_ready}, models={models_count}")
        
        dual_logger.info(f"Respuesta health check: {response_data}")
        
        return jsonify(response_data)
        
    except Exception as e:
        dual_logger.error(f"❌ ERROR EN HEALTH CHECK: {str(e)}")
        dual_logger.error(f"Tipo de error: {type(e).__name__}")
        ...
        dual_logger.error(f"Respuesta de error: {error_response}")
        return jsonify(error_response), 500

# Endpoint adicional simplificado para checks rápidos
@app.route('/health')
def simple_health_check():
    """Health check simplificado sin logging detallado (para monitoreo automático)"""
    try:
        status = 'ok' if (app_ready and predictor.is_ready()) else 'not_ready'
        return jsonify({
            'status': status,
            'timestamp': datetime.now().isoformat()
        })
...
# Endpoint para verificar el estado del sistema de logging
@app.route('/api/logging-status')
def logging_status():
    """Endpoint para verificar el estado del sistema de logging"""
    dual_logger.info("=== VERIFICACIÓN DE ESTADO DE LOGGING ===")
    dual_logger.info(f"IP del cliente: {request.remote_addr}")
    
    try:
        # Verificar estado de los loggers
        console_handler_active = any(isinstance(h, logging.StreamHandler) for h in dual_logger.logger.handlers)
        file_handler_active = any(isinstance(h, logging.FileHandler) for h in dual_logger.logger.handlers)
        
        logging_info = {
            'dual_logger_active': True,
            'console_logging': console_handler_active,
            'file_logging': file_handler_active,
            'log_level': dual_logger.logger.level,
            'handlers_count': len(dual_logger.logger.handlers),
            'timestamp': datetime.now().isoformat()
        }
        
        dual_logger.info(f"Estado del sistema de logging: {logging_info}")
        dual_logger.info("✅ Verificación de logging completada")
        
        return jsonify(logging_info)
        
    except Exception as e:
        dual_logger.error(f"❌ Error verificando estado de logging: {str(e)}")
        return jsonify({
            'status': 'error',
            'error': str(e),
            'timestamp': datetime.now().isoformat()
...
def test_logging():
    ...
    dual_logger.info("=== RUTA DE PRUEBA DE LOGGING ACCEDIDA ===")
    dual_logger.info(f"IP del cliente: {request.remote_addr}")
    dual_logger.info("Generando logs de prueba...")
    
    # Generar diferentes tipos de logs
    dual_logger.info("✅ Este es un mensaje de INFO de prueba")
    dual_logger.warning("⚠️ Este es un mensaje de WARNING de prueba")
    dual_logger.error("❌ Este es un mensaje de ERROR de prueba (no es un error real)")
    
    # Información del sistema
    dual_logger.info(f"Sistema operativo: {os.name}")
    dual_logger.info(f"Directorio actual: {os.getcwd()}")
    dual_logger.info(f"Variables de entorno configuradas: EMAIL_SENDER={bool(os.getenv('EMAIL_SENDER'))}")
    
    dual_logger.info("Logs de prueba completados")
    
    return jsonify({
        "message": "Logs de prueba generados correctamente",
        "timestamp": datetime.now().isoformat(),
        "logs_generated": [
            "1 INFO log de inicio",
            "1 INFO log con IP",
            "1 INFO log",
            "1 WARNING log", 
            "1 ERROR log (prueba)",
            "3 INFO logs del sistema",
            "1 INFO log de finalización"
        ],
        "total_logs": 8
    })

@app.route('/trigger-log-check')
def trigger_log_check():
    """Ruta para disparar manualmente el check de logs"""
    try:
        dual_logger.info("=== CHECK DE LOGS DISPARADO MANUALMENTE ===")
        dual_logger.info(f"IP del cliente: {request.remote_addr}")
        dual_logger.info("Ejecutando check_logs_and_send_email()...")
        
        check_logs_and_send_email()
        
        dual_logger.info("✅ Check manual de logs completado exitosamente")
        return jsonify({
            "message": "Check de logs ejecutado correctamente",
            "timestamp": datetime.now().isoformat(),
            "status": "success"
        })
    except Exception as e:
        dual_logger.error(f"❌ Error en check manual de logs: {str(e)}")
        return jsonify({
            "error": str(e),
            "timestamp": datetime.now().isoformat(),
            "status": "error"
...
if __name__ == "__main__":
    ...
    dual_logger.info("Iniciando monitoreo periódico de logs...")
    periodic_log_check()
